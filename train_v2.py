...
from models.modeling_resnet import ResNet, block
...
def valid(args, model, writer, phase, test_loader, global_step, KEYS, inner_loop_idx):
    ...
    for step, batch in enumerate(epoch_iterator):
        if args.dataset in ['MRI','MRI-BALANCED','MRI-BALANCED-3Classes', 'MRI-BALANCED-3Classes_2Val','MRI-BALANCED-3Classes_Nested', 'MRI-EQUAL', 'ffr']:
...
def train(args, model, KEYS, inner_loop_idx = None):
    """ Train the model """
    ...
    # Prepare dataset
    train_loader, val_loader, test_loader, _ = get_loader(args, inner_loop_idx)
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if not args.multi_stage_classification:
                # x = x.to(dtype=torch.long, device=args.device)
                x = x.type(torch.cuda.FloatTensor)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["cifar10", "cifar100", 'MRI', 'MRI-BALANCED', 'MRI-BALANCED-3Classes', 'MRI-BALANCED-3Classes_2Val', 'MRI-EQUAL', 'MRI-BALANCED-3Classes_Nested', 'ffr'], default="ffr",
    ...
    parser.add_argument("--num_fold", choices=[0, 1, 2, 3, 4], type=int, required=True,
    ...
    parser.add_argument("--model_type", choices=["ViT-B_16", "ViT-B_32", "ViT-L_16",
                                                 ...
                                                 "ViT-MRI", 'R50-ViT-MRI', "resnet"],
                        default="ViT-B_16",
                        help="Which variant to use.")
    parser.add_argument('--root_dir', type=str, default=os.path.join('data','dataset_SB3'))
    parser.add_argument('--split_path', type=str, default=os.path.join('data','5BalancedFoldValSplit_complete.json'))
    ...
    _, _, _, args.loss_weights = get_loader(args)
